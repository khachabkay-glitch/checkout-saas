...
function mask(key: string) {
  ...
  return key.slice(0, 8) + "â¢â¢â¢â¢" + key.slice(-4);
}

/* ââââââââââââââââââââââ COPY BUTTON ââââââââââââââââââââââ */
...
/* ââââââââââââââââââââââ ENV VAR LABELS ââââââââââââââââââââââ */
...
export default function AdminPage() {
  ...
  /* ââ Fetch revenue for a config ââ */
  ...
  /* ââ Fetch env vars for checkout tab ââ */
  ...
  /* ââ On auth â load configs ââ */
  useEffect(() => {
    if (authed) fetchConfigs();
  }, [authed, fetchConfigs]);

  /* ââ Fetch revenue for each config once ââ */
  useEffect(() => {
  ...
  /* ââ Load checkout data when switching to that tab ââ */
  useEffect(() => {
  ...
  /* ââ Save checkout env vars ââ */
  ...
  /* ââ Go live on a specific project ââ */
  ...
  return (
    <div style={s.page}>
      ...
      {/* ââââââââââââ THE OFFICE TAB ââââââââââââ */}
      {tab === "office" && (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  ...
                  <select value={form.target_project} onChange={(e) => setForm((f) => ({ ...f, target_project: e.target.value }))} style={{ ...s.input, cursor: "pointer" }}>
                    {PROJECT_LIST.map((p) => (
                      <option key={p} value={p}>{p} â {PROJECTS[p]}</option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <span style={{ fontSize: 10, color: "#999", fontWeight: 400 }}>
                        â {PROJECTS[c.target_project || "serravalle-checkout"] || c.target_project}
                      </span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      {rev ? (
                        <span style={{ fontSize: 13, color: "#666", fontWeight: 500 }}>
                          Today: {fmt(rev.today_revenue, rev.currency)}
                        </span>
                      ) : (
                    ...
                    <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 16 }}>
                      ...
                      {rev && (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, margin: "16px 0" }}>
                          <div>
                            <p style={s.statLabel}>Today&apos;s Revenue</p>
                            <p style={s.statValue}>{fmt(rev.today_revenue, rev.currency)}</p>
                          </div>
                          <div>
                            <p style={s.statLabel}>Orders Today</p>
                            <p style={s.statValue}>{rev.today_orders}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            <p style={s.statLabel}>Lifetime</p>
      ...
      {/* ââââââââââââ CURRENT CHECKOUT TAB ââââââââââââ */}
      {tab === "checkout" && (
      ...
      {tab === "office" && (
        <p style={{ fontSize: 11, color: "#bbb", textAlign: "center" as const, marginTop: 4 }}>
          Projects: {PROJECT_LIST.map((p) => `${p} â ${PROJECTS[p]}`).join(" Â· ")}
        </p>
      )}
    </div>
  );
